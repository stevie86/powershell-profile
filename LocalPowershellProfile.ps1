Write-Host "Loading profile: $($PROFILE) - $(Get-Date)"
...
function winutil {
    ...
    if ($winUtilScript) {
        Invoke-Expression $winUtilScript
    }
}

# Open WinUtil pre-release
function winutildev {
    $winUtilDevScript = Invoke-RestMethod https://christitus.com/windev
    if ($winUtilDevScript) {
        Invoke-Expression $winUtilDevScript
    }
}
...
function uptime {
    try {
        if (Get-Command Get-Uptime -ErrorAction SilentlyContinue) {
            $uptime = Get-Uptime | Select-Object -ExpandProperty Uptime
            Write-Host "Uptime: $($uptime.Days) days, $($uptime.Hours) hours, $($uptime.Minutes) minutes, $($uptime.Seconds) seconds" -ForegroundColor Blue
        } else {
...
function Get-Theme {
    if (Test-Path -Path $PROFILE.CurrentUserAllHosts -PathType leaf) {
        ...
        if ($null -ne $existingTheme -and $existingTheme -ne "") {
                    if ($existingTheme) {
                        Invoke-Expression $existingTheme
                        return
                    }
                }
                $themeInit = oh-my-posh init pwsh --config https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/main/themes/cobalt2.omp.json
                if ($themeInit) {
                    Invoke-Expression $themeInit
                }
            } else {
                $themeInit = oh-my-posh init pwsh --config https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/main/themes/cobalt2.omp.json
                if ($themeInit) {
...
if (Get-Command zoxide -ErrorAction SilentlyContinue) {
    $zoxideInit = (& { (zoxide init --cmd cd powershell | Out-String) })
    if ($zoxideInit) {
...
if (Test-Path "$PSScriptRoot\CTTcustom.ps1") {
    $cttCustomScript = Get-Content -Path "$PSScriptRoot\CTTcustom.ps1" -Raw
    if ($cttCustomScript) {
